 ...
 */
public class Tank extends Hooman {
    ...
    private static final int EVOLUTION_INDEX = 10;
    private static final int DAMAGE = 100; // Balanced damage
    private static final int RANGE = 400; // Balanced range
    private static final int SPLASH = 75; // Balanced splash damage radius
    private static final int RELOAD_SPEED = 3000; // Balanced reload speed
    private static final int COST = 300; // Balanced cost

    /**
     * Constructor for the Tank class.
     * 
     * @param position The position of the Tank.
     * @param isActive Whether the Tank is active.
     * @param isVisible Whether the Tank is visible.
     */
    public Tank(Point position, boolean isActive, boolean isVisible) {
    ...
    /**
     * Imports the sprites for the Tank.
     * 
     * @return An array of Images representing the Tank's sprites.
     */
    private static Image[] importSprites() {
        Image[] sprites = new Image[3];
        try {
            InputStream inputStream = Tank.class.getResourceAsStream("/Resources/tank.png");
            sprites[0] = ImageIO.read(inputStream);

            inputStream = Tank.class.getResourceAsStream("/Resources/tankTurret.png");
            sprites[1] = ImageIO.read(inputStream).getScaledInstance(WEAPON_SIZE, WEAPON_SIZE, Image.SCALE_AREA_AVERAGING);

            inputStream = Tank.class.getResourceAsStream("/Resources/tankShell.png");
    ...
    /**
     * Draws the Tank and its attack animation.
     * 
    ...
    public void draw(Graphics g) {
        ...
        // Rotate turret to face the alien
        g2dWeapon.rotate(rotateAngle + Math.PI / 2);

        // Draw the turret (offset it by negative currentX and currentY after translation)
